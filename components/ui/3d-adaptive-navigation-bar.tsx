...
export const PillBase: React.FC = () => {
  ...
  // Auth state listener
  useEffect(() => {
    // Check initial session (Local Mode only)
    const checkUser = () => {
        const localUser = localStorage.getItem('morado_user')
        setUser(localUser ? JSON.parse(localUser) : null)
    }
    
    checkUser()

    // Listen for custom storage events to detect login changes
  ...
  if (user) {
    navItems.push({ label: 'Sign Out', id: 'signout', path: '/' });
  } else {
    navItems.push({ label: 'Login', id: 'login', path: '/login' });
    navItems.push({ label: 'Sign Up', id: 'signup', path: '/register' });
  }
  ...
  useEffect(() => {
    if (hovering) {
      ...
      const targetWidth = user ? 580 : 640; 
  ...
  const handleSectionClick = async (sectionId: string, path: string) => {
    // Handle Sign Out
    if (sectionId === 'signout') {
      localStorage.removeItem('morado_user')

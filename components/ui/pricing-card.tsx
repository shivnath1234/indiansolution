...
export const PricingCard: React.FC<PricingCardProps> = ({ tier, paymentFrequency }) => {
  ...
  return (
    <Card
      className={cn(
        "relative flex flex-col gap-8 overflow-hidden p-6 transition-all duration-300 ease-out hover:scale-[1.02] hover:shadow-xl hover:border-primary/50",
        isHighlighted
        ...
        {typeof price === "number" ? (
          <>
            ...
            <p className="-mt-2 text-xs text-muted-foreground opacity-80">
               {paymentFrequency === "yearly" ? "Per month, billed yearly" : "Per month"}
            </p>
          </>
        ) : (
